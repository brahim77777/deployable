...
def maxz(table, output='summary'):
    ...
    while next_round(table):
        old_table = table.copy()  # Store the current state of the table
        table = pivot(loc_piv(table)[0], loc_piv(table)[1], table)

        # Check if the current state has been encountered before
        if tuple(table.flatten()) in table_states:
            return "unbound"  # Or break out of the loop
        else:
            table_states.add(tuple(table.flatten()))  # Add current state to the set
...
if __name__ == "__main__":
    ...
    constrain(m,'-2,3,L,12')
    constrain(m,'2,-1,L,12')
    obj(m,'1,3,0')
    # print(maxz(m))

    # print("primal(min)----------------", end="\n\n\t")
    m = gen_matrix(2,3)
    constrain(m,'3,1,G,10')
    constrain(m,'-4,3,G,-8')
    constrain(m,'1,-1,G,-6')
    obj(m,'-2,9,0')
    # print(minz(m))
    ...
    # print("dual(max)-----------------------------", end="\n\n\t")
    m = gen_matrix(3,2)
    constrain(m,'3,-4,-1,L,-2')
    constrain(m,'1,3,1,L,9')
    obj(m,'10,-8,-6,0')
    # print(maxz(m))

    # for i in sys.argv : print("this is argv: ", i)
    ...
    obj(m,sys.argv[-1])
    if problem_type == "max" :
        print(json.dumps(maxz(m)))
    elif problem_type == "min" :
        print(json.dumps(minz(m)))
    else : print("problem type should be max or min")
